...
String const& String::getSubstring(int pos, int len) const
{
	...
	int n = (pos - 1) / MAX_STRING_CHUNK_LEN;
	pos = (pos - 1) % MAX_STRING_CHUNK_LEN;
	pos++;
	//(bef)pos : 99 100 101 200
	// n       :  0   0   1   1
	//(aft)pos : 99 100   1 100


	String const* pstr = this;
	while (n--)
		pstr = pstr->next; //go to the string chunk that the pos locates

	if (pos + len -1 <= MAX_STRING_CHUNK_LEN)
	{//only need one string chunk
		memcpy(&strSubStr->str[1], &pstr->str[pos], len*sizeof(char));
		::setLength(strSubStr->str, len);
		strSubStr->str[len + 1] = '\0';
	}
	else
	{//to be copied part goes over the current string chunk's end
		//copy str1[()()(o)(o)] -> copy str2[(x)(x)()()] -> copy str2[()()(o)(o)] -> copy str3[(x)(x)()()] -> ...
		//n = (len - 1) / MAX_STRING_CHUNK_LEN;
		int partLen = MAX_STRING_CHUNK_LEN - pos + 1;
		int remainLen = len;
		String *newPrevStr = strSubStr;
		String *newStr = strSubStr;
		int srcStringIndex = pos;
		...
		do
		{
			...
			{
				newStr = new String();
				newStr->prev = newPrevStr;
				newPrevStr->next = newStr;
			}
			::increaseLengthBy(newStr->str,partLen);
			memcpy(&newStr->str[subStringIndex], &pstr->str[srcStringIndex], partLen*sizeof(char));
			remainLen = len - partLen;
			
			pstr = pstr->next;
			partLen = MAX_STRING_CHUNK_LEN - partLen; //A->B->A->B... (A+B == MAX_STRING_CHUNK_LEN)
			srcStringIndex = (pos + 1) - srcStringIndex; //[pos]->[1]->[pos]->[1]...
			subStringIndex = (((MAX_STRING_CHUNK_LEN - pos + 1) + 1) - subStringIndex + 1)%MAX_STRING_CHUNK_LEN; //[1]->[firstPartLen + 1]->[1]->[firstPartLen + 1]->..
			if (isFullStringChunk(newStr->str))
			{
				newStr->str[MAX_STRING_CHUNK_LEN + 1] = '\0';
				newPrevStr = newStr;
				newStr = newStr->next;
			}
		} while (remainLen > MAX_STRING_CHUNK_LEN);
		if (remainLen != 0)
		{
			if (!newStr)
			{
			...
			::increaseLengthBy(newStr->str, remainLen);
			memcpy(&newStr->str[subStringIndex], &pstr->str[srcStringIndex], remainLen*sizeof(char));
		}
	}
